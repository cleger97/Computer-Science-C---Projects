 ...
 *Edited by Alex Leger (added a lot of new functions.)
...
public:
  ...
  // Name - PreviewCard()
  // Desc - Shows the card without removing it.
  // Preconditions - A hand with at least one card and a valid index
  // Postconditions - Returns the card *without* removing it from the array.
  Card PreviewCard(int index); 
  ...
  // Name - DisplayDefuses()
  // Desc - Function to show all your possible defuse cards
  // Preconditions - None
  // Postconditions - Uses cout to print defuse card options
  void DisplayDefuses();
  ...
  // Desc - Checks a deck for a card of the same type. Returns index if one exists
  //        or -1 otherwise.
  ...
  // Name - SetPhase(int type)
  // Desc - Sets the phase of player. 0 - allow play. 1 - draw phase. 2 - potential defuse. 3 - end turn.
  // Preconditions - It is this player's turn and valid input is sent.
  // Postconditions - None.
  void SetPhase(int type);

  // Name - GetPhase()
  // Desc - Get the phase.
  // Precondition - Phase is initialized.
  // Postcondition - Returns the phase.
  int GetPhase();
